...
export const authenticateUser = action => dispatch => {
  ...
  axios.post("https://supportsysrestfulapi.herokuapp.com/user/login", {
    email,
    password
  })
    .then(response => {
...
export const registerUser = action => dispatch => {
  ...
  axios.post("https://supportsysrestfulapi.herokuapp.com/user/register", {
    firstName,
    lastName,
    emailAddress: email,
    password,
...
export const activateAccount = action => dispatch => {
  ...
  axios.put(`https://supportsysrestfulapi.herokuapp.com/user/activate/new-account/${accountId}`)
    .then(response => {
      if (response.data.msg) {
        dispatch({
...
export const fetchReceivedResponses = action => dispatch => {
  ...
  axios.get(`https://supportsysrestfulapi.herokuapp.com/user/read/user-support/responses/${_id}`, {
    headers: {
...
export const submitSupport = action => dispatch => {
  ...
  axios.post("https://supportsysrestfulapi.herokuapp.com/user/submit/user-support/", {
    userId: _id,
    subject,
    body,
    private: private_
...
export const viewResponse = action => dispatch => {
  ...
  axios.get(`https://supportsysrestfulapi.herokuapp.com/user/view/user-support-response/${id}`, {
    headers: {
      authorization: `bearer ${token}`
    }
  })
...
export const logoutUser = action => dispatch => {
  ...
  axios.post("https://supportsysrestfulapi.herokuapp.com/user/logout", {}, {
    headers: {
      authorization: `bearer ${token}`
    }
  })
